...
class _ChatScreenState extends ConsumerState<ChatScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    if ((isCustomer && auth.valueOrNull!.role.toLowerCase() != 'customer') ||
        (!isCustomer && auth.valueOrNull!.role.toLowerCase() != 'provider')) {
      return AuthenticationPrompt(message: l10n.chatAccessDenied);
    }
    if (widget.requestId.isEmpty || widget.providerId.isEmpty)
      return _ChatError(message: l10n.conversationUnavailable, onRetry: null);
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            onPressed: () => ref.invalidate(chatControllerProvider(query)),
            tooltip: l10n.retry,
      ...
      body: state.isLoading
          ...
          ? _ChatError(
              ...
              onRetry: () => ref.invalidate(chatControllerProvider(query)),
            )
          : Column(
              children: [
                _ChatContextHeader(
                ...
                if (state.otherIsTyping)
                  _TypingIndicator(
                    name: widget.role == ChatParticipantRole.customer
                        ? state.conversation!.provider.name
                        : state.conversation!.customer.name,
                    l10n: l10n,
                  ),
                _Composer(
                  ...
                  l10n: l10n,
                  onChanged: (value) => ref
                      .read(chatControllerProvider(query).notifier)
  ...
  Future<void> _send(ChatQuery query) async {
    ...
    if (text.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(AppLocalizations.of(context)!.messageRequired)),
      );
      return;
    }
  ...
  String _errorMessage(AppLocalizations l10n, Object error) {
    if (error is ChatException) {
      switch (error.code) {
        case ChatFailureCode.invalidContext:
          return l10n.conversationUnavailable;
        case ChatFailureCode.accessDenied:
          return l10n.chatAccessDenied;
        case ChatFailureCode.conversationUnavailable:
          return l10n.conversationUnavailable;
        case ChatFailureCode.unknown:
        ...
        children: [
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text(provider.name, style: AppTextStyles.heading3),
                const SizedBox(height: AppSpacing.xs),
                Text(
                  l10n.requestReference(conversation!.requestId),
...
class _MessageList extends StatelessWidget {
  ...
  Widget build(BuildContext context) => ListView.builder(
    ...
    itemBuilder: (context, index) => _MessageBubble(
      ...
      isMine: messages[index].senderRole == role,
    ),
  );
}
          ...
          decoration: BoxDecoration(
            ...
            borderRadius: BorderRadiusDirectional.only(
              ...
              bottomStart: Radius.circular(
                isMine ? AppSizes.radiusLg : AppSizes.radiusSm,
              ),
              bottomEnd: Radius.circular(
                isMine ? AppSizes.radiusSm : AppSizes.radiusLg,
              ),
            ),
            ...
            children: [
              ...
              Text(
                ...
                style: AppTextStyles.caption.copyWith(
                  color: isMine ? AppColors.white70 : AppColors.textLight,
                ),
              ),
            ],
          ),
      ...
      child: Row(
        children: [
          ...
          if (status == ChatConnectionStatus.failed ||
            ...
            TextButton(onPressed: onRetry, child: Text(l10n.chatReconnect)),
        ],
      ),
    );
  }
    ...
    child: Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(l10n.emptyConversation, style: AppTextStyles.heading3),
...
class _ChatLoadingView extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Column(
    children: [
      Container(height: 78, color: AppColors.card),
      const Expanded(
        child: Center(
